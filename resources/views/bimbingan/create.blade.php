        ...
        <form action="{{ route('bimbingan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="dosen_id" required class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:border-blue-500">
                ...
                <div>
                    ...
                    <input type="date" name="tgl_bimbingan" value="{{ old('tgl_bimbingan', date('Y-m-d')) }}" required
                        class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:border-blue-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="topik_bimbingan" value="{{ old('topik_bimbingan') }}" required placeholder="misal: Perbaikan Metodologi Penelitian & Instrumen Kuesioner"
                        class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:border-blue-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="uraian_mahasiswa" rows="4" required placeholder="Tuliskan poin-poin yang sudah dikerjakan atau pertanyaan/masalah yang dikonsultasikan kepada dosen..."
                    class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs sm:text-sm focus:outline-none focus:border-blue-500">{{ old('uraian_mahasiswa') }}</textarea>
            </div>
