...
<body class="h-full flex items-center justify-center p-4 font-sans antialiased bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950">
    ...
    <div class="w-full max-w-lg">
        ...
        <div class="text-center mb-6">
            <div class="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-tr from-blue-600 to-cyan-400 text-white shadow-xl shadow-blue-500/20 mb-3 ring-8 ring-blue-500/10">
                <i class="fa-solid fa-user-plus text-2xl"></i>
            </div>
            <h1 class="text-2xl font-black tracking-tight text-white">Registrasi Akun Mahasiswa</h1>
            <p class="text-xs text-slate-400 mt-1">STIKes Panti Waluya Malang &bull; SIMTA & Yudisium</p>
        </div>
            ...
            <form action="{{ route('register.post') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required
                        placeholder="misal: Maria Fransiska"
                        class="w-full px-4 py-2.5 bg-slate-950 border border-slate-700/80 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="nim" value="{{ old('nim') }}" required
                            placeholder="misal: 220101001"
                            class="w-full px-4 py-2.5 bg-slate-950 border border-slate-700/80 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5">Program Studi</label>
                        <select name="prodi_id" required class="w-full px-4 py-2.5 bg-slate-950 border border-slate-700/80 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5">Email Mahasiswa</label>
                    <input type="email" name="email" value="{{ old('email') }}" required
                        placeholder="misal: mhs@stikespantiwaluya.ac.id"
                        class="w-full px-4 py-2.5 bg-slate-950 border border-slate-700/80 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                ...
                <button type="submit" class="w-full py-3 px-4 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl text-sm shadow-lg shadow-blue-600/30 transition duration-200 mt-4">
                    Daftar Sekarang <i class="fa-solid fa-arrow-right ml-1.5"></i>
                </button>
            </form>

            <div class="mt-6 text-center text-xs text-slate-400">
                Sudah punya akun? <a href="{{ route('login') }}" class="text-cyan-400 font-semibold hover:underline">Masuk disini</a>
            </div>
