...
<div class="space-y-6">
    ...
    <!-- Header Welcome -->
    <div class="bg-gradient-to-r from-slate-900 via-sky-950 to-slate-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <span class="px-3 py-1 text-xs font-semibold rounded-full bg-sky-500/20 text-sky-300 border border-sky-500/30">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('bimbingan.index') }}" class="px-4 py-2.5 bg-sky-600 hover:bg-sky-500 text-white font-bold rounded-xl text-xs shadow-lg transition">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm">
            <div class="flex items-center justify-between mb-5">
                ...
                <a href="{{ route('bimbingan.index') }}" class="text-xs font-bold text-sky-600">Lihat Semua</a>
            </div>
                ...
                <div class="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="space-y-1">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="px-2 py-0.5 rounded bg-sky-100 text-sky-700 text-[10px] font-bold uppercase">{{ $log->bab }}</span>
                        </div>
                    ...
                    <div>
                        <a href="{{ route('bimbingan.index') }}" class="inline-flex items-center px-3 py-1.5 bg-sky-50 text-sky-700 hover:bg-sky-600 hover:text-white rounded-xl text-xs font-bold transition">
            ...
            <div class="space-y-3">
                ...
                <div class="p-3.5 rounded-2xl bg-slate-50 border border-slate-100 text-xs space-y-1.5">
                    <div class="flex items-center justify-between">
                        <span class="font-bold text-sky-700 uppercase">{{ $s->jenis }}</span>
                    ...
                    <div class="pt-1 flex items-center justify-between text-[11px]">
                        ...
                        <a href="{{ route('sidang.show', $s->id) }}" class="font-bold text-sky-600 hover:underline">Form Nilai &rarr;</a>
                    </div>
                </div>
                @empty
