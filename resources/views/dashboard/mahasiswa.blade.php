...
<div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-5 gap-3">
            ...
            <div class="p-3.5 rounded-2xl border {{ $currentStep >= 5 ? 'bg-emerald-50/50 border-emerald-200 text-emerald-900' : 'bg-slate-50 border-slate-200 text-slate-400' }}">
                <div class="flex items-center gap-2 mb-1.5">
                    ...
                    <span class="text-xs font-bold">Yudisium & SKL</span>
                </div>
                <p class="text-[11px] {{ $currentStep >= 5 ? 'text-emerald-700' : 'text-slate-400' }}">Bebas Tanggungan & SK</p>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <!-- Right: Status Yudisium & SKL Card -->
        <div class="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm space-y-4">
            <h3 class="font-bold text-base text-slate-800">Status Yudisium & SKL</h3>
            ...
            <div class="space-y-3 text-xs">
                <div class="p-4 rounded-2xl {{ $pendaftaranYudisium->status === 'lulus' ? 'bg-emerald-50 border border-emerald-200 text-emerald-900' : 'bg-amber-50 border border-amber-200 text-amber-900' }}">
                    <div class="font-bold text-sm flex items-center justify-between">
                        ...
                        <span class="uppercase">{{ $pendaftaranYudisium->status }}</span>
                    </div>
                    <p class="mt-1 text-[11px]">{{ $pendaftaranYudisium->periodeYudisium->nama_periode }}</p>
                </div>

                @if($pendaftaranYudisium->status === 'lulus')
                <div class="space-y-2 pt-2">
                    <p class="text-[11px] text-slate-500">Predikat: <strong class="text-slate-800">{{ $pendaftaranYudisium->predikat }}</strong></p>
                    <p class="text-[11px] text-slate-500">No. SK: <strong class="text-slate-800 font-mono">{{ $pendaftaranYudisium->nomor_sk }}</strong></p>
                    
                    <a href="{{ route('yudisium.cetak-skl', $pendaftaranYudisium->id) }}" target="_blank" class="w-full mt-3 py-2.5 px-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/30 transition">
                        <i class="fa-solid fa-print"></i> Unduh / Cetak SKL Digital
                    </a>
                </div>
                @else
                <a href="{{ route('yudisium.index') }}" class="block w-full text-center py-2 px-3 bg-slate-800 text-white font-bold rounded-xl text-xs">
                    Kelola Berkas Yudisium &rarr;
                </a>
                @endif
            </div>
            @else
