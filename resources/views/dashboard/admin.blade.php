...
<div class="space-y-6">
    ...
    <!-- Welcome Banner -->
    <div class="relative overflow-hidden bg-gradient-to-r from-slate-900 via-slate-800 to-emerald-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl">
        <div class="relative z-10 max-w-2xl">
            ...
            <p class="text-slate-300 text-sm mt-2 leading-relaxed">
                Kelola pendaftaran judul tugas akhir, plotting dosen pembimbing, jadwal sidang sempro/skripsi, hingga validasi yudisium dalam satu sistem terintegrasi.
            </p>
        </div>
        <div class="absolute right-4 bottom-0 opacity-10 text-9xl">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            <div>
                ...
                <span class="text-xs text-emerald-600 font-semibold"><i class="fa-solid fa-circle-check"></i> Terdaftar</span>
            </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            <div>
                ...
                <span class="text-xs text-sky-600 font-semibold"><i class="fa-solid fa-user-tie"></i> Aktif Menguji</span>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm">
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h3 class="font-bold text-base text-slate-800">Pengajuan & Progress TA Terbaru</h3>
                    <p class="text-xs text-slate-400">Daftar judul tugas akhir mahasiswa semester ini</p>
                </div>
                <a href="{{ route('tugas-akhir.index') }}" class="text-xs font-bold text-emerald-600 hover:text-emerald-700">
                ...
                <div class="py-3.5 flex items-start justify-between gap-4">
                    <div class="space-y-1">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[11px] text-slate-400 font-medium">({{ $ta->mahasiswa->nim }})</span>
                        </div>
                    ...
                    <div class="shrink-0 text-right">
                        ...
                        <div class="mt-1">
                            <a href="{{ route('tugas-akhir.show', $ta->id) }}" class="text-[11px] font-semibold text-slate-400 hover:text-emerald-600">Detail &rarr;</a>
                        </div>
                    </div>
                </div>
                @empty
                <p class="text-xs text-slate-400 py-4 text-center">Belum ada data tugas akhir.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('sidang.index') }}" class="text-xs font-semibold text-emerald-600">Kelola</a>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="p-3 rounded-2xl bg-slate-50 border border-slate-100 text-xs">
                        <div class="flex items-center justify-between font-bold text-slate-800 mb-1">
                            <span class="uppercase text-emerald-700">{{ $sidang->jenis }}</span>
                    ...
                    @empty
                    <p class="text-xs text-slate-400 py-3 text-center">Tidak ada jadwal sidang terdekat.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('pengumuman.index') }}" class="text-xs font-semibold text-emerald-600">Buat Baru</a>
                </div>

                <div class="space-y-3">
                    @forelse($pengumumans as $p)
                    <div class="p-3 rounded-2xl bg-emerald-50/50 border border-emerald-100/80 text-xs space-y-1">
                        <div class="flex items-center justify-between">
                            <span class="font-bold text-emerald-800">{{ $p->judul }}</span>
                            @if($p->is_pinned)
                    ...
                    @empty
                    <p class="text-xs text-slate-400 text-center py-2">Belum ada pengumuman.</p>
                    @endforelse
                </div>
            </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm">
        <div class="flex items-center justify-between mb-5">
            ...
            <a href="{{ route('yudisium.index') }}" class="text-xs font-bold text-emerald-600 hover:text-emerald-700">
        ...
        <div class="divide-y divide-slate-100">
            ...
            <div class="py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div>
                    <a href="{{ route('yudisium.show', $b->pendaftaran_yudisium_id) }}" class="inline-flex items-center px-3 py-1.5 bg-amber-50 text-amber-700 hover:bg-emerald-600 hover:text-white rounded-xl text-xs font-bold transition">
                        <i class="fa-solid fa-clipboard-check mr-1.5"></i> Periksa Berkas & Tetapkan SK
                    </a>
                </div>
            </div>
            @empty
