        ...
        <form action="{{ route('tugas-akhir.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="judul" rows="3" required placeholder="Tuliskan judul skripsi secara lengkap dan jelas..."
                    class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs sm:text-sm focus:outline-none focus:border-blue-500 transition">{{ old('judul', $existingTA->judul ?? '') }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" name="bidang_kajian" value="{{ old('bidang_kajian', $existingTA->bidang_kajian ?? '') }}" required
                    placeholder="misal: Keperawatan Medikal Bedah, Farmakologi Klinik, Rekam Medis Elektronik"
                    class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:border-blue-500 transition">
            </div>
            ...
            <div>
                ...
                <textarea name="abstrak" rows="5" required placeholder="Jelaskan secara singkat latar belakang masalah, urgensi, metode yang digunakan, serta target luaran yang diharapkan..."
                    class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs sm:text-sm focus:outline-none focus:border-blue-500 transition">{{ old('abstrak', $existingTA->abstrak ?? '') }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="usulan_pembimbing1_id" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:border-blue-500 transition">
