...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm">
        ...
        <p class="text-xs text-slate-500 mt-1">Daftar seluruh mahasiswa yang terdaftar dalam sistem SIMTA & Yudisium.</p>
    </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80">
                        <td class="px-6 py-4">
                            ...
                            <div class="font-mono text-emerald-600 font-bold">{{ $m->nim }}</div>
                            <div class="text-[10px] text-slate-400">{{ $m->user->email ?? '-' }}</div>
                        </td>
                        <td class="px-6 py-4 font-semibold text-slate-800">
                            {{ $m->prodi->nama_prodi ?? 'TI' }} ({{ $m->prodi->jenjang ?? 'S1' }})
                        </td>
                        ...
                        <td class="px-6 py-4">
                            @if($m->tugasAkhir)
                            <span class="inline-block px-2.5 py-1 text-[10px] font-bold rounded-lg uppercase bg-emerald-100 text-emerald-700">
