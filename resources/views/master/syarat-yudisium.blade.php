...
<div class="space-y-6" x-data="{ modalAdd: false }">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-sm space-y-3 hover:shadow-md transition">
            <div class="flex items-center justify-between">
                <span class="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 text-[10px] font-extrabold uppercase">
                    {{ $s->kategori }}
                </span>
            ...
            <div class="pt-2 border-t border-slate-100 flex items-center justify-between text-xs font-bold">
                <span class="text-slate-500">Status Kewajiban:</span>
                @if($s->is_wajib)
        ...
        <div @click.away="modalAdd = false" class="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl space-y-4">
            ...
            <form action="{{ route('master.syarat-yudisium.store') }}" method="POST" class="space-y-4 text-xs">
                ...
                <div>
                    ...
                    <input type="text" name="nama_syarat" required placeholder="misal: Surat Bebas Perpustakaan & Upload Repository" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-blue-500">
                </div>

                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block font-bold text-slate-700 uppercase mb-1">Kode Unik *</label>
                        <input type="text" name="kode_syarat" required placeholder="misal: BEBAS_PERPUS" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-mono uppercase focus:outline-none focus:border-blue-500">
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="3" placeholder="Jelaskan detail berkas yang harus diunggah..." class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-blue-500"></textarea>
                </div>

                <div class="pt-2">
                    <label class="flex items-center gap-2 cursor-pointer font-bold text-slate-700">
                        <input type="checkbox" name="is_wajib" value="1" checked class="w-4 h-4 text-blue-600 rounded">
                ...
                <div class="pt-3 flex justify-end gap-2 border-t border-slate-100">
                    ...
                    <button type="submit" class="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl shadow-md shadow-blue-600/30 transition">Simpan Syarat</button>
                </div>
            </form>
        </div>
    </div>
